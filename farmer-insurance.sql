...
ROUND(SUM(SumInsured) / SUM(TotalPopulation), 3) AS total_population_ratio
 ...
 Returnonly those districts where the highest amount exceeds 20 crores.18. Perform a LEFT JOIN 
...
/*dvanced SQL Functions (Window Functions) [10 marks]22. UseROW_NUMBER()to rank records ordered by the
...
/*ata Integrity (Constraints, Foreign Keys) [4 marks]25. Create a tabledistrictswithDistrictCodeas the primary key and columns forDistrictNameandStateCode. Create another tablestateswithStateCodeas primary key and column forStateName.2
